...
class SpokenInterface {
    onComand = async (event: IpcMainEvent, command: SpokenCommand | null, ...args: unknown[]) => {
        if (command == null) {
            return event.reply('Spoken:executeCommandResult', { err: 404 });
        }
        ...
        if (err) {
            ...
            return event.reply('Spoken:executeCommandResult', { error: err instanceof Error ? err.message : 'Unknown error' });
        }
    ...
    private async execute(command: SpokenCommand, parent?: string): Promise<[object | null, Error | null]> {
        try {
            ...
            console.error('[wrapper.SpokenInterface.onCommand] Error executing command: ', err instanceof Error ? err.message : err);
