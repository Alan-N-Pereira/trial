...
const useAzureVoiceRecognition: VoiceRecognitionHook = () => {
    ...
    useEffect(() => {
        ...
        recognizer
            .on('results', (result: SpeechSDK.SpeechRecognitionResult, isFinal: boolean) => {
                ...
                if (isFinal) {
                    const match = findCommand(result, language);
    ...
    const analyzeSentence = async (phrase: string, timeout: number | null = 3000) => {
        const sanitizedText = sanitizePonctuation(phrase, language);
        const match = findCommand({ text: sanitizedText } as SpeechSDK.SpeechRecognitionResult, language);
...
function findCommand(voiceToTextResponse: SpeechSDK.SpeechRecognitionResult, language: string) {
