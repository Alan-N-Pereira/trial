...
const perfectSquare = {
	...
	placeholder: {
		'pt-BR':
			'Teste se um número é um quadrado perfeito.<br/><br/><i class="fa fa-info-circle"></i> É possível dizer "escreva ..." para escrever qualquer coisa na linha atual.',
		'en-US':
			'Test if a number is a perfect square.<br/><br/><i class="fa fa-info-circle"></i> It\'s possible to say "write ..." to write anything in the current line.'
	},
	statement: {
...
export const concretize = (index, lang) => ({
	get id() {
	...
	get title() {
		return problems[index].title[lang];
	},
	get placeholder() {
		return problems[index].placeholder[lang];
	},
	get statement() {
		return problems[index].statement[lang];
	},
	get solution() {
		return problems[index].solution[lang];
	},
	get code() {
		return (problems[index] && problems[index].code && problems[index].code[lang]) || '';
	}
});
